package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.CellularData124: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.CellularData124",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 24f,
        viewportHeight = 24f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(20f, 5f)
            curveToRelative(0.552f, 0f, 1f, 0.447f, 1f, 1f)
            verticalLineToRelative(13f)
            curveToRelative(0f, 0.553f, -0.448f, 1f, -1f, 1f)
            reflectiveCurveToRelative(-1f, -0.447f, -1f, -1f)
            verticalLineTo(6f)
            curveToRelative(0f, -0.553f, 0.448f, -1f, 1f, -1f)
            close()
            moveToRelative(-4f, 3f)
            curveToRelative(0.552f, 0f, 1f, 0.446f, 1f, 0.995f)
            verticalLineToRelative(10.01f)
            curveTo(17f, 19.555f, 16.552f, 20f, 16f, 20f)
            reflectiveCurveToRelative(-1f, -0.446f, -1f, -0.995f)
            verticalLineTo(8.995f)
            curveTo(15f, 8.445f, 15.448f, 8f, 16f, 8f)
            close()
            moveToRelative(-4f, 3f)
            curveToRelative(0.552f, 0f, 1f, 0.448f, 1f, 1f)
            verticalLineToRelative(7f)
            curveToRelative(0f, 0.552f, -0.448f, 1f, -1f, 1f)
            reflectiveCurveToRelative(-1f, -0.448f, -1f, -1f)
            verticalLineToRelative(-7f)
            curveToRelative(0f, -0.552f, 0.448f, -1f, 1f, -1f)
            close()
            moveToRelative(-4f, 3f)
            curveToRelative(0.552f, 0f, 1f, 0.445f, 1f, 0.994f)
            verticalLineToRelative(4.012f)
            curveTo(9f, 19.555f, 8.552f, 20f, 8f, 20f)
            reflectiveCurveToRelative(-1f, -0.445f, -1f, -0.994f)
            verticalLineToRelative(-4.012f)
            curveTo(7f, 14.445f, 7.448f, 14f, 8f, 14f)
            close()
            moveToRelative(-4f, 3f)
            curveToRelative(0.552f, 0f, 1f, 0.44f, 1f, 0.984f)
            verticalLineToRelative(1.032f)
            curveTo(5f, 19.56f, 4.552f, 20f, 4f, 20f)
            reflectiveCurveToRelative(-1f, -0.44f, -1f, -0.984f)
            verticalLineToRelative(-1.032f)
            curveTo(3f, 17.44f, 3.448f, 17f, 4f, 17f)
            close()
        }
    }.build()
}
