package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.ChannelShare16: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.ChannelShare16",
        defaultWidth = 16.dp,
        defaultHeight = 16.dp,
        viewportWidth = 16f,
        viewportHeight = 16f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(1f, 3.5f)
            curveTo(1f, 2.12f, 2.12f, 1f, 3.5f, 1f)
            horizontalLineToRelative(4f)
            curveTo(8.88f, 1f, 10f, 2.12f, 10f, 3.5f)
            verticalLineTo(4f)
            horizontalLineTo(9f)
            verticalLineTo(3.5f)
            curveTo(9f, 2.672f, 8.328f, 2f, 7.5f, 2f)
            horizontalLineToRelative(-4f)
            curveTo(2.672f, 2f, 2f, 2.672f, 2f, 3.5f)
            verticalLineToRelative(4f)
            curveTo(2f, 8.328f, 2.672f, 9f, 3.5f, 9f)
            horizontalLineToRelative(5.585f)
            curveTo(9.291f, 8.417f, 9.847f, 8f, 10.5f, 8f)
            curveTo(11.328f, 8f, 12f, 8.672f, 12f, 9.5f)
            reflectiveCurveTo(11.328f, 11f, 10.5f, 11f)
            curveToRelative(-0.653f, 0f, -1.209f, -0.417f, -1.415f, -1f)
            horizontalLineTo(3.5f)
            curveTo(2.12f, 10f, 1f, 8.88f, 1f, 7.5f)
            verticalLineToRelative(-4f)
            close()
            moveTo(6f, 11f)
            verticalLineToRelative(0.5f)
            curveTo(6f, 12.88f, 7.12f, 14f, 8.5f, 14f)
            horizontalLineToRelative(4f)
            curveToRelative(1.38f, 0f, 2.5f, -1.12f, 2.5f, -2.5f)
            verticalLineToRelative(-4f)
            curveTo(15f, 6.12f, 13.88f, 5f, 12.5f, 5f)
            horizontalLineTo(6.915f)
            curveTo(6.709f, 4.417f, 6.153f, 4f, 5.5f, 4f)
            curveTo(4.672f, 4f, 4f, 4.672f, 4f, 5.5f)
            reflectiveCurveTo(4.672f, 7f, 5.5f, 7f)
            curveToRelative(0.653f, 0f, 1.209f, -0.417f, 1.415f, -1f)
            horizontalLineTo(12.5f)
            curveTo(13.328f, 6f, 14f, 6.672f, 14f, 7.5f)
            verticalLineToRelative(4f)
            curveToRelative(0f, 0.828f, -0.672f, 1.5f, -1.5f, 1.5f)
            horizontalLineToRelative(-4f)
            curveTo(7.672f, 13f, 7f, 12.328f, 7f, 11.5f)
            verticalLineTo(11f)
            horizontalLineTo(6f)
            close()
        }
    }.build()
}
