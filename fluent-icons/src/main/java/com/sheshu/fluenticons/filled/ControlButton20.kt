package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.ControlButton20: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.ControlButton20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(2f, 6.75f)
            curveTo(2f, 5.231f, 3.231f, 4f, 4.75f, 4f)
            horizontalLineToRelative(10.5f)
            curveTo(16.769f, 4f, 18f, 5.231f, 18f, 6.75f)
            verticalLineToRelative(6.5f)
            curveToRelative(0f, 1.519f, -1.231f, 2.75f, -2.75f, 2.75f)
            horizontalLineTo(4.75f)
            curveTo(3.231f, 16f, 2f, 14.769f, 2f, 13.25f)
            verticalLineToRelative(-6.5f)
            close()
            moveTo(6f, 7f)
            curveTo(4.895f, 7f, 4f, 7.895f, 4f, 9f)
            verticalLineToRelative(2f)
            curveToRelative(0f, 1.105f, 0.895f, 2f, 2f, 2f)
            horizontalLineToRelative(0.5f)
            curveTo(6.776f, 13f, 7f, 12.776f, 7f, 12.5f)
            reflectiveCurveTo(6.776f, 12f, 6.5f, 12f)
            horizontalLineTo(6f)
            curveToRelative(-0.552f, 0f, -1f, -0.448f, -1f, -1f)
            verticalLineTo(9f)
            curveToRelative(0f, -0.552f, 0.448f, -1f, 1f, -1f)
            horizontalLineToRelative(0.5f)
            curveTo(6.776f, 8f, 7f, 7.776f, 7f, 7.5f)
            reflectiveCurveTo(6.776f, 7f, 6.5f, 7f)
            horizontalLineTo(6f)
            close()
            moveToRelative(3f, 0.5f)
            curveTo(9f, 7.224f, 8.776f, 7f, 8.5f, 7f)
            reflectiveCurveTo(8f, 7.224f, 8f, 7.5f)
            verticalLineTo(9f)
            horizontalLineTo(7.5f)
            curveTo(7.224f, 9f, 7f, 9.224f, 7f, 9.5f)
            reflectiveCurveTo(7.224f, 10f, 7.5f, 10f)
            horizontalLineTo(8f)
            verticalLineToRelative(1.75f)
            curveTo(8f, 12.44f, 8.56f, 13f, 9.25f, 13f)
            horizontalLineTo(9.5f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            reflectiveCurveTo(9.776f, 12f, 9.5f, 12f)
            horizontalLineTo(9.25f)
            curveTo(9.112f, 12f, 9f, 11.888f, 9f, 11.75f)
            verticalLineTo(10f)
            horizontalLineToRelative(0.5f)
            curveTo(9.776f, 10f, 10f, 9.776f, 10f, 9.5f)
            reflectiveCurveTo(9.776f, 9f, 9.5f, 9f)
            horizontalLineTo(9f)
            verticalLineTo(7.5f)
            close()
            moveToRelative(7f, 0f)
            curveTo(16f, 7.224f, 15.776f, 7f, 15.5f, 7f)
            reflectiveCurveTo(15f, 7.224f, 15f, 7.5f)
            verticalLineToRelative(5f)
            curveToRelative(0f, 0.276f, 0.224f, 0.5f, 0.5f, 0.5f)
            reflectiveCurveToRelative(0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-5f)
            close()
            moveToRelative(-4f, 3f)
            curveToRelative(0f, -0.276f, 0.224f, -0.5f, 0.5f, -0.5f)
            horizontalLineToRelative(1f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            reflectiveCurveTo(13.776f, 9f, 13.5f, 9f)
            horizontalLineToRelative(-1f)
            curveTo(11.672f, 9f, 11f, 9.672f, 11f, 10.5f)
            verticalLineToRelative(2f)
            curveToRelative(0f, 0.276f, 0.224f, 0.5f, 0.5f, 0.5f)
            reflectiveCurveToRelative(0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-2f)
            close()
        }
    }.build()
}
