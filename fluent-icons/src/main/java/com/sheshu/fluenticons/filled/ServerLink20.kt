package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.ServerLink20: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.ServerLink20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(7.5f, 2f)
            curveTo(6.12f, 2f, 5f, 3.12f, 5f, 4.5f)
            verticalLineToRelative(11f)
            curveTo(5f, 16.88f, 6.12f, 18f, 7.5f, 18f)
            horizontalLineToRelative(1.837f)
            curveTo(9.12f, 17.545f, 9f, 17.037f, 9f, 16.5f)
            reflectiveCurveToRelative(0.12f, -1.045f, 0.337f, -1.5f)
            horizontalLineTo(7.5f)
            curveTo(7.224f, 15f, 7f, 14.776f, 7f, 14.5f)
            reflectiveCurveTo(7.224f, 14f, 7.5f, 14f)
            horizontalLineToRelative(2.55f)
            curveToRelative(0.632f, -0.619f, 1.496f, -1f, 2.45f, -1f)
            horizontalLineTo(15f)
            verticalLineTo(4.5f)
            curveTo(15f, 3.12f, 13.88f, 2f, 12.5f, 2f)
            horizontalLineToRelative(-5f)
            close()
            moveToRelative(5f, 11f)
            horizontalLineToRelative(-5f)
            curveTo(7.224f, 13f, 7f, 12.776f, 7f, 12.5f)
            reflectiveCurveTo(7.224f, 12f, 7.5f, 12f)
            horizontalLineToRelative(5f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(12.776f, 13f, 12.5f, 13f)
            close()
            moveTo(7f, 5.5f)
            curveTo(7f, 5.224f, 7.224f, 5f, 7.5f, 5f)
            horizontalLineToRelative(5f)
            curveTo(12.776f, 5f, 13f, 5.224f, 13f, 5.5f)
            reflectiveCurveTo(12.776f, 6f, 12.5f, 6f)
            horizontalLineToRelative(-5f)
            curveTo(7.224f, 6f, 7f, 5.776f, 7f, 5.5f)
            close()
            moveToRelative(5.5f, 8.5f)
            curveToRelative(-1.38f, 0f, -2.5f, 1.12f, -2.5f, 2.5f)
            reflectiveCurveToRelative(1.12f, 2.5f, 2.5f, 2.5f)
            horizontalLineTo(13f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            reflectiveCurveTo(13.276f, 18f, 13f, 18f)
            horizontalLineToRelative(-0.5f)
            curveToRelative(-0.828f, 0f, -1.5f, -0.672f, -1.5f, -1.5f)
            reflectiveCurveToRelative(0.672f, -1.5f, 1.5f, -1.5f)
            horizontalLineTo(13f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            reflectiveCurveTo(13.276f, 14f, 13f, 14f)
            horizontalLineToRelative(-0.5f)
            close()
            moveToRelative(3.5f, 0f)
            curveToRelative(-0.276f, 0f, -0.5f, 0.224f, -0.5f, 0.5f)
            reflectiveCurveTo(15.724f, 15f, 16f, 15f)
            horizontalLineToRelative(0.5f)
            curveToRelative(0.828f, 0f, 1.5f, 0.672f, 1.5f, 1.5f)
            reflectiveCurveTo(17.328f, 18f, 16.5f, 18f)
            horizontalLineTo(16f)
            curveToRelative(-0.276f, 0f, -0.5f, 0.224f, -0.5f, 0.5f)
            reflectiveCurveTo(15.724f, 19f, 16f, 19f)
            horizontalLineToRelative(0.5f)
            curveToRelative(1.38f, 0f, 2.5f, -1.12f, 2.5f, -2.5f)
            reflectiveCurveTo(17.88f, 14f, 16.5f, 14f)
            horizontalLineTo(16f)
            close()
            moveToRelative(-4f, 2.5f)
            curveToRelative(0f, -0.276f, 0.224f, -0.5f, 0.5f, -0.5f)
            horizontalLineToRelative(4f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(16.776f, 17f, 16.5f, 17f)
            horizontalLineToRelative(-4f)
            curveToRelative(-0.276f, 0f, -0.5f, -0.224f, -0.5f, -0.5f)
            close()
        }
    }.build()
}
