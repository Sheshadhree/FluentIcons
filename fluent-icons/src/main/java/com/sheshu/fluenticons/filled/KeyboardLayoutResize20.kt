package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.KeyboardLayoutResize20: ImageVector by
        lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.KeyboardLayoutResize20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(3f, 3.5f)
            curveTo(3f, 3.224f, 3.224f, 3f, 3.5f, 3f)
            horizontalLineToRelative(7f)
            curveTo(10.776f, 3f, 11f, 2.776f, 11f, 2.5f)
            reflectiveCurveTo(10.776f, 2f, 10.5f, 2f)
            horizontalLineToRelative(-7f)
            curveTo(2.672f, 2f, 2f, 2.672f, 2f, 3.5f)
            verticalLineToRelative(4f)
            curveTo(2f, 7.776f, 2.224f, 8f, 2.5f, 8f)
            reflectiveCurveTo(3f, 7.776f, 3f, 7.5f)
            verticalLineToRelative(-4f)
            close()
            moveTo(3.5f, 9f)
            curveTo(2.672f, 9f, 2f, 9.672f, 2f, 10.5f)
            verticalLineToRelative(6f)
            curveTo(2f, 17.328f, 2.672f, 18f, 3.5f, 18f)
            horizontalLineToRelative(8f)
            curveToRelative(0.828f, 0f, 1.5f, -0.672f, 1.5f, -1.5f)
            verticalLineToRelative(-6f)
            curveTo(13f, 9.672f, 12.328f, 9f, 11.5f, 9f)
            horizontalLineToRelative(-8f)
            close()
            moveToRelative(2f, 6f)
            horizontalLineToRelative(4f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(9.776f, 16f, 9.5f, 16f)
            horizontalLineToRelative(-4f)
            curveTo(5.224f, 16f, 5f, 15.776f, 5f, 15.5f)
            reflectiveCurveTo(5.224f, 15f, 5.5f, 15f)
            close()
            moveToRelative(0f, -2.75f)
            curveTo(5.5f, 12.664f, 5.164f, 13f, 4.75f, 13f)
            reflectiveCurveTo(4f, 12.664f, 4f, 12.25f)
            reflectiveCurveToRelative(0.336f, -0.75f, 0.75f, -0.75f)
            reflectiveCurveToRelative(0.75f, 0.336f, 0.75f, 0.75f)
            close()
            moveToRelative(2f, 0.75f)
            curveToRelative(-0.414f, 0f, -0.75f, -0.336f, -0.75f, -0.75f)
            reflectiveCurveTo(7.086f, 11.5f, 7.5f, 11.5f)
            reflectiveCurveToRelative(0.75f, 0.336f, 0.75f, 0.75f)
            reflectiveCurveTo(7.914f, 13f, 7.5f, 13f)
            close()
            moveToRelative(3.5f, -0.75f)
            curveToRelative(0f, 0.414f, -0.336f, 0.75f, -0.75f, 0.75f)
            reflectiveCurveTo(9.5f, 12.664f, 9.5f, 12.25f)
            reflectiveCurveToRelative(0.336f, -0.75f, 0.75f, -0.75f)
            reflectiveCurveTo(11f, 11.836f, 11f, 12.25f)
            close()
            moveTo(17.5f, 9f)
            curveTo(17.776f, 9f, 18f, 9.224f, 18f, 9.5f)
            verticalLineToRelative(7f)
            curveToRelative(0f, 0.828f, -0.672f, 1.5f, -1.5f, 1.5f)
            horizontalLineToRelative(-2f)
            curveToRelative(-0.276f, 0f, -0.5f, -0.224f, -0.5f, -0.5f)
            reflectiveCurveToRelative(0.224f, -0.5f, 0.5f, -0.5f)
            horizontalLineToRelative(2f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-7f)
            curveTo(17f, 9.224f, 17.224f, 9f, 17.5f, 9f)
            close()
            moveToRelative(-4f, -7f)
            curveTo(13.224f, 2f, 13f, 2.224f, 13f, 2.5f)
            reflectiveCurveTo(13.224f, 3f, 13.5f, 3f)
            horizontalLineToRelative(2.793f)
            lineToRelative(-4.147f, 4.146f)
            curveToRelative(-0.195f, 0.196f, -0.195f, 0.512f, 0f, 0.708f)
            curveToRelative(0.196f, 0.195f, 0.512f, 0.195f, 0.708f, 0f)
            lineTo(17f, 3.707f)
            verticalLineTo(6.5f)
            curveTo(17f, 6.776f, 17.224f, 7f, 17.5f, 7f)
            reflectiveCurveTo(18f, 6.776f, 18f, 6.5f)
            verticalLineToRelative(-4f)
            curveTo(18f, 2.224f, 17.776f, 2f, 17.5f, 2f)
            horizontalLineToRelative(-4f)
            close()
        }
    }.build()
}
