package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.Bug20: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.Bug20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(9f, 2.5f)
            curveTo(9f, 2.224f, 8.776f, 2f, 8.5f, 2f)
            reflectiveCurveTo(8f, 2.224f, 8f, 2.5f)
            verticalLineTo(3f)
            curveToRelative(0f, 0.397f, 0.116f, 0.767f, 0.315f, 1.079f)
            curveTo(6.988f, 4.388f, 6f, 5.579f, 6f, 7f)
            horizontalLineTo(5.5f)
            curveTo(4.672f, 7f, 4f, 6.328f, 4f, 5.5f)
            verticalLineToRelative(-2f)
            curveTo(4f, 3.224f, 3.776f, 3f, 3.5f, 3f)
            reflectiveCurveTo(3f, 3.224f, 3f, 3.5f)
            verticalLineToRelative(2f)
            curveTo(3f, 6.88f, 4.12f, 8f, 5.5f, 8f)
            horizontalLineTo(6f)
            verticalLineToRelative(1.5f)
            horizontalLineTo(2.5f)
            curveTo(2.224f, 9.5f, 2f, 9.724f, 2f, 10f)
            reflectiveCurveToRelative(0.224f, 0.5f, 0.5f, 0.5f)
            horizontalLineTo(6f)
            verticalLineTo(12f)
            horizontalLineTo(5.5f)
            curveTo(4.12f, 12f, 3f, 13.12f, 3f, 14.5f)
            verticalLineToRelative(2f)
            curveTo(3f, 16.776f, 3.224f, 17f, 3.5f, 17f)
            reflectiveCurveTo(4f, 16.776f, 4f, 16.5f)
            verticalLineToRelative(-2f)
            curveTo(4f, 13.672f, 4.672f, 13f, 5.5f, 13f)
            horizontalLineTo(6f)
            curveToRelative(0f, 2.21f, 1.79f, 4f, 4f, 4f)
            reflectiveCurveToRelative(4f, -1.79f, 4f, -4f)
            horizontalLineToRelative(0.5f)
            curveToRelative(0.828f, 0f, 1.5f, 0.672f, 1.5f, 1.5f)
            verticalLineToRelative(2f)
            curveToRelative(0f, 0.276f, 0.224f, 0.5f, 0.5f, 0.5f)
            reflectiveCurveToRelative(0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-2f)
            curveToRelative(0f, -1.38f, -1.12f, -2.5f, -2.5f, -2.5f)
            horizontalLineTo(14f)
            verticalLineToRelative(-1.5f)
            horizontalLineToRelative(3.5f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            reflectiveCurveToRelative(-0.224f, -0.5f, -0.5f, -0.5f)
            horizontalLineTo(14f)
            verticalLineTo(8f)
            horizontalLineToRelative(0.5f)
            curveTo(15.88f, 8f, 17f, 6.88f, 17f, 5.5f)
            verticalLineToRelative(-2f)
            curveTo(17f, 3.224f, 16.776f, 3f, 16.5f, 3f)
            reflectiveCurveTo(16f, 3.224f, 16f, 3.5f)
            verticalLineToRelative(2f)
            curveTo(16f, 6.328f, 15.328f, 7f, 14.5f, 7f)
            horizontalLineTo(14f)
            curveToRelative(0f, -1.421f, -0.988f, -2.612f, -2.315f, -2.921f)
            curveTo(11.885f, 3.767f, 12f, 3.397f, 12f, 3f)
            verticalLineTo(2.5f)
            curveTo(12f, 2.224f, 11.776f, 2f, 11.5f, 2f)
            reflectiveCurveTo(11f, 2.224f, 11f, 2.5f)
            verticalLineTo(3f)
            curveToRelative(0f, 0.552f, -0.448f, 1f, -1f, 1f)
            reflectiveCurveTo(9f, 3.552f, 9f, 3f)
            verticalLineTo(2.5f)
            close()
        }
    }.build()
}
