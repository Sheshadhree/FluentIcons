package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.FilmstripPlay16: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.FilmstripPlay16",
        defaultWidth = 16.dp,
        defaultHeight = 16.dp,
        viewportWidth = 16f,
        viewportHeight = 16f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(3.5f, 3f)
            curveTo(2.12f, 3f, 1f, 4.12f, 1f, 5.5f)
            verticalLineToRelative(5f)
            curveTo(1f, 11.88f, 2.12f, 13f, 3.5f, 13f)
            horizontalLineToRelative(9f)
            curveToRelative(1.38f, 0f, 2.5f, -1.12f, 2.5f, -2.5f)
            verticalLineToRelative(-5f)
            curveTo(15f, 4.12f, 13.88f, 3f, 12.5f, 3f)
            horizontalLineToRelative(-9f)
            close()
            moveToRelative(9f, 2f)
            curveTo(12.776f, 5f, 13f, 5.224f, 13f, 5.5f)
            verticalLineToRelative(1f)
            curveTo(13f, 6.776f, 12.776f, 7f, 12.5f, 7f)
            reflectiveCurveTo(12f, 6.776f, 12f, 6.5f)
            verticalLineToRelative(-1f)
            curveTo(12f, 5.224f, 12.224f, 5f, 12.5f, 5f)
            close()
            moveTo(12f, 9.5f)
            curveTo(12f, 9.224f, 12.224f, 9f, 12.5f, 9f)
            reflectiveCurveTo(13f, 9.224f, 13f, 9.5f)
            verticalLineToRelative(1f)
            curveToRelative(0f, 0.276f, -0.224f, 0.5f, -0.5f, 0.5f)
            reflectiveCurveTo(12f, 10.776f, 12f, 10.5f)
            verticalLineToRelative(-1f)
            close()
            moveTo(3.5f, 5f)
            curveTo(3.776f, 5f, 4f, 5.224f, 4f, 5.5f)
            verticalLineToRelative(1f)
            curveTo(4f, 6.776f, 3.776f, 7f, 3.5f, 7f)
            reflectiveCurveTo(3f, 6.776f, 3f, 6.5f)
            verticalLineToRelative(-1f)
            curveTo(3f, 5.224f, 3.224f, 5f, 3.5f, 5f)
            close()
            moveTo(3f, 9.5f)
            curveTo(3f, 9.224f, 3.224f, 9f, 3.5f, 9f)
            reflectiveCurveTo(4f, 9.224f, 4f, 9.5f)
            verticalLineToRelative(1f)
            curveTo(4f, 10.776f, 3.776f, 11f, 3.5f, 11f)
            reflectiveCurveTo(3f, 10.776f, 3f, 10.5f)
            verticalLineToRelative(-1f)
            close()
            moveToRelative(4f, -3f)
            curveToRelative(0f, -0.385f, 0.346f, -0.626f, 0.62f, -0.43f)
            lineToRelative(2.206f, 1.56f)
            curveToRelative(0.233f, 0.165f, 0.233f, 0.573f, 0f, 0.738f)
            lineTo(7.621f, 9.93f)
            curveTo(7.346f, 10.126f, 7f, 9.885f, 7f, 9.499f)
            verticalLineTo(6.501f)
            close()
        }
    }.build()
}
