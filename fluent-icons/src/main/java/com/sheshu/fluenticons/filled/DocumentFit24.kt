package com.sheshu.fluenticons.filled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Filled.DocumentFit24: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Filled.DocumentFit24",
        defaultWidth = 24.dp,
        defaultHeight = 24.dp,
        viewportWidth = 24f,
        viewportHeight = 24f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(6.25f, 2f)
            curveTo(5.007f, 2f, 4f, 3.007f, 4f, 4.25f)
            verticalLineToRelative(15.5f)
            curveTo(4f, 20.993f, 5.007f, 22f, 6.25f, 22f)
            horizontalLineToRelative(11.5f)
            curveToRelative(1.243f, 0f, 2.25f, -1.007f, 2.25f, -2.25f)
            verticalLineTo(4.25f)
            curveTo(20f, 3.007f, 18.993f, 2f, 17.75f, 2f)
            horizontalLineTo(6.25f)
            close()
            moveTo(9f, 6.5f)
            curveTo(8.724f, 6.5f, 8.5f, 6.724f, 8.5f, 7f)
            verticalLineToRelative(1.25f)
            curveTo(8.5f, 8.664f, 8.164f, 9f, 7.75f, 9f)
            reflectiveCurveTo(7f, 8.664f, 7f, 8.25f)
            verticalLineTo(7f)
            curveToRelative(0f, -1.105f, 0.895f, -2f, 2f, -2f)
            horizontalLineToRelative(1.25f)
            curveTo(10.664f, 5f, 11f, 5.336f, 11f, 5.75f)
            reflectiveCurveTo(10.664f, 6.5f, 10.25f, 6.5f)
            horizontalLineTo(9f)
            close()
            moveToRelative(0f, 11f)
            horizontalLineToRelative(1.25f)
            curveToRelative(0.414f, 0f, 0.75f, 0.336f, 0.75f, 0.75f)
            reflectiveCurveTo(10.664f, 19f, 10.25f, 19f)
            horizontalLineTo(9f)
            curveToRelative(-1.105f, 0f, -2f, -0.895f, -2f, -2f)
            verticalLineToRelative(-1.25f)
            curveTo(7f, 15.336f, 7.336f, 15f, 7.75f, 15f)
            reflectiveCurveToRelative(0.75f, 0.336f, 0.75f, 0.75f)
            verticalLineTo(17f)
            curveToRelative(0f, 0.276f, 0.224f, 0.5f, 0.5f, 0.5f)
            close()
            moveTo(15.5f, 7f)
            curveToRelative(0f, -0.276f, -0.224f, -0.5f, -0.5f, -0.5f)
            horizontalLineToRelative(-1.25f)
            curveTo(13.336f, 6.5f, 13f, 6.164f, 13f, 5.75f)
            reflectiveCurveTo(13.336f, 5f, 13.75f, 5f)
            horizontalLineTo(15f)
            curveToRelative(1.105f, 0f, 2f, 0.895f, 2f, 2f)
            verticalLineToRelative(1.25f)
            curveTo(17f, 8.664f, 16.664f, 9f, 16.25f, 9f)
            reflectiveCurveTo(15.5f, 8.664f, 15.5f, 8.25f)
            verticalLineTo(7f)
            close()
            moveTo(15f, 17.5f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-1.25f)
            curveToRelative(0f, -0.414f, 0.336f, -0.75f, 0.75f, -0.75f)
            reflectiveCurveTo(17f, 15.336f, 17f, 15.75f)
            verticalLineTo(17f)
            curveToRelative(0f, 1.105f, -0.895f, 2f, -2f, 2f)
            horizontalLineToRelative(-1.25f)
            curveTo(13.336f, 19f, 13f, 18.664f, 13f, 18.25f)
            reflectiveCurveToRelative(0.336f, -0.75f, 0.75f, -0.75f)
            horizontalLineTo(15f)
            close()
        }
    }.build()
}
