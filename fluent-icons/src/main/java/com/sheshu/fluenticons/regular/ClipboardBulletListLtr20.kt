package com.sheshu.fluenticons.regular

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Regular.ClipboardBulletListLtr20: ImageVector by
        lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Regular.ClipboardBulletListLtr20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(9f, 8.5f)
            curveTo(9f, 8.224f, 9.224f, 8f, 9.5f, 8f)
            horizontalLineTo(13f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(13.276f, 9f, 13f, 9f)
            horizontalLineTo(9.5f)
            curveTo(9.224f, 9f, 9f, 8.776f, 9f, 8.5f)
            close()
            moveToRelative(0f, 3f)
            curveTo(9f, 11.224f, 9.224f, 11f, 9.5f, 11f)
            horizontalLineTo(13f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(13.276f, 12f, 13f, 12f)
            horizontalLineTo(9.5f)
            curveTo(9.224f, 12f, 9f, 11.776f, 9f, 11.5f)
            close()
            moveToRelative(0f, 3f)
            curveTo(9f, 14.224f, 9.224f, 14f, 9.5f, 14f)
            horizontalLineTo(13f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(13.276f, 15f, 13f, 15f)
            horizontalLineTo(9.5f)
            curveTo(9.224f, 15f, 9f, 14.776f, 9f, 14.5f)
            close()
            moveToRelative(-1f, -6f)
            curveToRelative(0f, 0.414f, -0.336f, 0.75f, -0.75f, 0.75f)
            reflectiveCurveTo(6.5f, 8.914f, 6.5f, 8.5f)
            reflectiveCurveToRelative(0.336f, -0.75f, 0.75f, -0.75f)
            reflectiveCurveTo(8f, 8.086f, 8f, 8.5f)
            close()
            moveToRelative(0f, 3f)
            curveToRelative(0f, 0.414f, -0.336f, 0.75f, -0.75f, 0.75f)
            reflectiveCurveTo(6.5f, 11.914f, 6.5f, 11.5f)
            reflectiveCurveToRelative(0.336f, -0.75f, 0.75f, -0.75f)
            reflectiveCurveTo(8f, 11.086f, 8f, 11.5f)
            close()
            moveToRelative(-0.75f, 3.75f)
            curveTo(7.664f, 15.25f, 8f, 14.914f, 8f, 14.5f)
            reflectiveCurveToRelative(-0.336f, -0.75f, -0.75f, -0.75f)
            reflectiveCurveTo(6.5f, 14.086f, 6.5f, 14.5f)
            reflectiveCurveToRelative(0.336f, 0.75f, 0.75f, 0.75f)
            close()
            moveTo(7.085f, 3f)
            curveTo(7.291f, 2.417f, 7.847f, 2f, 8.5f, 2f)
            horizontalLineToRelative(3f)
            curveToRelative(0.653f, 0f, 1.209f, 0.417f, 1.415f, 1f)
            horizontalLineTo(14.5f)
            curveTo(15.328f, 3f, 16f, 3.672f, 16f, 4.5f)
            verticalLineToRelative(12f)
            curveToRelative(0f, 0.828f, -0.672f, 1.5f, -1.5f, 1.5f)
            horizontalLineToRelative(-9f)
            curveTo(4.672f, 18f, 4f, 17.328f, 4f, 16.5f)
            verticalLineToRelative(-12f)
            curveTo(4f, 3.672f, 4.672f, 3f, 5.5f, 3f)
            horizontalLineToRelative(1.585f)
            close()
            moveTo(8.5f, 3f)
            curveTo(8.224f, 3f, 8f, 3.224f, 8f, 3.5f)
            reflectiveCurveTo(8.224f, 4f, 8.5f, 4f)
            horizontalLineToRelative(3f)
            curveTo(11.776f, 4f, 12f, 3.776f, 12f, 3.5f)
            reflectiveCurveTo(11.776f, 3f, 11.5f, 3f)
            horizontalLineToRelative(-3f)
            close()
            moveTo(7.085f, 4f)
            horizontalLineTo(5.5f)
            curveTo(5.224f, 4f, 5f, 4.224f, 5f, 4.5f)
            verticalLineToRelative(12f)
            curveTo(5f, 16.776f, 5.224f, 17f, 5.5f, 17f)
            horizontalLineToRelative(9f)
            curveToRelative(0.276f, 0f, 0.5f, -0.224f, 0.5f, -0.5f)
            verticalLineToRelative(-12f)
            curveTo(15f, 4.224f, 14.776f, 4f, 14.5f, 4f)
            horizontalLineToRelative(-1.585f)
            curveToRelative(-0.206f, 0.583f, -0.762f, 1f, -1.415f, 1f)
            horizontalLineToRelative(-3f)
            curveTo(7.847f, 5f, 7.291f, 4.583f, 7.085f, 4f)
            close()
        }
    }.build()
}
