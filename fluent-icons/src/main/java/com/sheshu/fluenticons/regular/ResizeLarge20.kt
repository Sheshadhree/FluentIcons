package com.sheshu.fluenticons.regular

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Regular.ResizeLarge20: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Regular.ResizeLarge20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(5.5f, 4f)
            curveTo(4.672f, 4f, 4f, 4.672f, 4f, 5.5f)
            verticalLineToRelative(1f)
            curveTo(4f, 6.776f, 3.776f, 7f, 3.5f, 7f)
            reflectiveCurveTo(3f, 6.776f, 3f, 6.5f)
            verticalLineToRelative(-1f)
            curveTo(3f, 4.12f, 4.12f, 3f, 5.5f, 3f)
            horizontalLineToRelative(1f)
            curveTo(6.776f, 3f, 7f, 3.224f, 7f, 3.5f)
            reflectiveCurveTo(6.776f, 4f, 6.5f, 4f)
            horizontalLineToRelative(-1f)
            close()
            moveTo(16f, 5.5f)
            curveTo(16f, 4.672f, 15.328f, 4f, 14.5f, 4f)
            horizontalLineToRelative(-1f)
            curveTo(13.224f, 4f, 13f, 3.776f, 13f, 3.5f)
            reflectiveCurveTo(13.224f, 3f, 13.5f, 3f)
            horizontalLineToRelative(1f)
            curveTo(15.88f, 3f, 17f, 4.12f, 17f, 5.5f)
            verticalLineToRelative(1f)
            curveTo(17f, 6.776f, 16.776f, 7f, 16.5f, 7f)
            reflectiveCurveTo(16f, 6.776f, 16f, 6.5f)
            verticalLineToRelative(-1f)
            close()
            moveToRelative(0f, 9f)
            curveToRelative(0f, 0.828f, -0.672f, 1.5f, -1.5f, 1.5f)
            horizontalLineToRelative(-1f)
            curveToRelative(-0.276f, 0f, -0.5f, 0.224f, -0.5f, 0.5f)
            reflectiveCurveToRelative(0.224f, 0.5f, 0.5f, 0.5f)
            horizontalLineToRelative(1f)
            curveToRelative(1.38f, 0f, 2.5f, -1.12f, 2.5f, -2.5f)
            verticalLineToRelative(-1f)
            curveToRelative(0f, -0.276f, -0.224f, -0.5f, -0.5f, -0.5f)
            reflectiveCurveTo(16f, 13.224f, 16f, 13.5f)
            verticalLineToRelative(1f)
            close()
            moveToRelative(-12f, 0f)
            curveTo(4f, 15.328f, 4.672f, 16f, 5.5f, 16f)
            horizontalLineToRelative(1.25f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(7.026f, 17f, 6.75f, 17f)
            horizontalLineTo(5.5f)
            curveTo(4.12f, 17f, 3f, 15.88f, 3f, 14.5f)
            verticalLineToRelative(-1.25f)
            curveToRelative(0f, -0.276f, 0.224f, -0.5f, 0.5f, -0.5f)
            reflectiveCurveTo(4f, 12.974f, 4f, 13.25f)
            verticalLineToRelative(1.25f)
            close()
            moveTo(6.5f, 5f)
            curveTo(5.672f, 5f, 5f, 5.672f, 5f, 6.5f)
            verticalLineToRelative(7f)
            curveTo(5f, 14.328f, 5.672f, 15f, 6.5f, 15f)
            horizontalLineToRelative(7f)
            curveToRelative(0.828f, 0f, 1.5f, -0.672f, 1.5f, -1.5f)
            verticalLineToRelative(-7f)
            curveTo(15f, 5.672f, 14.328f, 5f, 13.5f, 5f)
            horizontalLineToRelative(-7f)
            close()
            moveTo(6f, 6.5f)
            curveTo(6f, 6.224f, 6.224f, 6f, 6.5f, 6f)
            horizontalLineToRelative(7f)
            curveTo(13.776f, 6f, 14f, 6.224f, 14f, 6.5f)
            verticalLineToRelative(7f)
            curveToRelative(0f, 0.276f, -0.224f, 0.5f, -0.5f, 0.5f)
            horizontalLineToRelative(-7f)
            curveTo(6.224f, 14f, 6f, 13.776f, 6f, 13.5f)
            verticalLineToRelative(-7f)
            close()
        }
    }.build()
}
