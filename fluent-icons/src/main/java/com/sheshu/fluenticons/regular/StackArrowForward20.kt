package com.sheshu.fluenticons.regular

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Regular.StackArrowForward20: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Regular.StackArrowForward20",
        defaultWidth = 20.dp,
        defaultHeight = 20.dp,
        viewportWidth = 20f,
        viewportHeight = 20f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(14.5f, 10f)
            curveTo(12.015f, 10f, 10f, 7.985f, 10f, 5.5f)
            reflectiveCurveTo(12.015f, 1f, 14.5f, 1f)
            reflectiveCurveTo(19f, 3.015f, 19f, 5.5f)
            reflectiveCurveTo(16.985f, 10f, 14.5f, 10f)
            close()
            moveToRelative(1.604f, -7.104f)
            curveToRelative(-0.196f, -0.195f, -0.512f, -0.195f, -0.708f, 0f)
            curveToRelative(-0.195f, 0.196f, -0.195f, 0.512f, 0f, 0.708f)
            lineTo(16.293f, 4.5f)
            horizontalLineTo(14.75f)
            curveTo(13.231f, 4.5f, 12f, 5.731f, 12f, 7.25f)
            verticalLineTo(7.5f)
            curveTo(12f, 7.776f, 12.224f, 8f, 12.5f, 8f)
            reflectiveCurveTo(13f, 7.776f, 13f, 7.5f)
            verticalLineTo(7.25f)
            curveToRelative(0f, -0.966f, 0.784f, -1.75f, 1.75f, -1.75f)
            horizontalLineToRelative(1.543f)
            lineToRelative(-0.897f, 0.896f)
            curveToRelative(-0.195f, 0.196f, -0.195f, 0.512f, 0f, 0.708f)
            curveToRelative(0.196f, 0.195f, 0.512f, 0.195f, 0.708f, 0f)
            lineToRelative(1.752f, -1.753f)
            curveToRelative(0.047f, -0.047f, 0.082f, -0.101f, 0.106f, -0.16f)
            curveTo(17.986f, 5.132f, 18f, 5.068f, 18f, 5f)
            reflectiveCurveToRelative(-0.014f, -0.132f, -0.038f, -0.191f)
            curveToRelative(-0.024f, -0.06f, -0.06f, -0.115f, -0.108f, -0.163f)
            lineToRelative(-1.75f, -1.75f)
            close()
            moveTo(14f, 11f)
            verticalLineToRelative(-0.022f)
            curveToRelative(-0.343f, -0.031f, -0.678f, -0.094f, -1f, -0.185f)
            verticalLineTo(11f)
            curveToRelative(0f, 0.552f, -0.448f, 1f, -1f, 1f)
            horizontalLineTo(4f)
            curveToRelative(-0.552f, 0f, -1f, -0.448f, -1f, -1f)
            verticalLineTo(5f)
            curveToRelative(0f, -0.552f, 0.448f, -1f, 1f, -1f)
            horizontalLineToRelative(5.207f)
            curveTo(9.306f, 3.651f, 9.437f, 3.317f, 9.6f, 3f)
            horizontalLineTo(4f)
            curveTo(2.895f, 3f, 2f, 3.895f, 2f, 5f)
            verticalLineToRelative(6f)
            curveToRelative(0f, 1.105f, 0.895f, 2f, 2f, 2f)
            horizontalLineToRelative(8f)
            curveToRelative(1.105f, 0f, 2f, -0.895f, 2f, -2f)
            close()
            moveToRelative(1f, -0.022f)
            curveToRelative(0.343f, -0.031f, 0.678f, -0.094f, 1f, -0.185f)
            verticalLineTo(11f)
            curveToRelative(0f, 2.21f, -1.79f, 4f, -4f, 4f)
            horizontalLineTo(6f)
            curveToRelative(-0.74f, 0f, -1.387f, -0.402f, -1.732f, -1f)
            horizontalLineTo(12f)
            curveToRelative(1.657f, 0f, 3f, -1.343f, 3f, -3f)
            verticalLineToRelative(-0.022f)
            close()
            moveToRelative(3f, -1.235f)
            curveToRelative(-0.307f, 0.253f, -0.642f, 0.474f, -1f, 0.657f)
            verticalLineTo(11f)
            curveToRelative(0f, 2.761f, -2.239f, 5f, -5f, 5f)
            horizontalLineTo(6.268f)
            curveTo(6.613f, 16.598f, 7.26f, 17f, 8f, 17f)
            horizontalLineToRelative(4f)
            curveToRelative(3.314f, 0f, 6f, -2.686f, 6f, -6f)
            verticalLineTo(9.743f)
            close()
        }
    }.build()
}
