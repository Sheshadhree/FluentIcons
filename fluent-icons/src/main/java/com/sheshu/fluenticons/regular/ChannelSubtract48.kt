package com.sheshu.fluenticons.regular

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Regular.ChannelSubtract48: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Regular.ChannelSubtract48",
        defaultWidth = 48.dp,
        defaultHeight = 48.dp,
        viewportWidth = 48f,
        viewportHeight = 48f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(6.189f, 10.72f)
            curveTo(6.87f, 8.008f, 9.326f, 6f, 12.25f, 6f)
            horizontalLineToRelative(23.5f)
            curveTo(39.202f, 6f, 42f, 8.798f, 42f, 12.25f)
            verticalLineToRelative(11.794f)
            curveToRelative(-0.78f, -0.5f, -1.618f, -0.919f, -2.5f, -1.244f)
            verticalLineTo(12.25f)
            curveToRelative(0f, -2.071f, -1.679f, -3.75f, -3.75f, -3.75f)
            horizontalLineToRelative(-23.5f)
            curveToRelative(-1.528f, 0f, -2.842f, 0.914f, -3.427f, 2.224f)
            curveTo(8.41f, 10.58f, 7.963f, 10.5f, 7.5f, 10.5f)
            curveToRelative(-0.46f, 0f, -0.9f, 0.077f, -1.311f, 0.22f)
            close()
            moveToRelative(16.61f, 28.78f)
            curveToRelative(0.326f, 0.882f, 0.745f, 1.72f, 1.245f, 2.5f)
            horizontalLineTo(12.25f)
            curveTo(8.798f, 42f, 6f, 39.202f, 6f, 35.75f)
            verticalLineTo(18.21f)
            curveToRelative(0.463f, 0.187f, 0.97f, 0.29f, 1.5f, 0.29f)
            curveToRelative(0.345f, 0f, 0.68f, -0.044f, 1f, -0.126f)
            verticalLineTo(35.75f)
            curveToRelative(0f, 2.071f, 1.679f, 3.75f, 3.75f, 3.75f)
            horizontalLineTo(22.8f)
            close()
            moveToRelative(0.94f, -11f)
            curveToRelative(0.526f, -0.909f, 1.158f, -1.748f, 1.88f, -2.5f)
            horizontalLineTo(17.25f)
            curveTo(16.56f, 26f, 16f, 26.56f, 16f, 27.25f)
            reflectiveCurveToRelative(0.56f, 1.25f, 1.25f, 1.25f)
            horizontalLineToRelative(6.49f)
            close()
            moveTo(6f, 12.5f)
            curveToRelative(-0.607f, 0.456f, -1f, 1.182f, -1f, 2f)
            curveToRelative(0f, 0.818f, 0.393f, 1.544f, 1f, 2f)
            curveTo(6.418f, 16.814f, 6.937f, 17f, 7.5f, 17f)
            curveToRelative(0.356f, 0f, 0.694f, -0.074f, 1f, -0.208f)
            curveToRelative(0.883f, -0.386f, 1.5f, -1.267f, 1.5f, -2.292f)
            reflectiveCurveToRelative(-0.617f, -1.906f, -1.5f, -2.292f)
            curveTo(8.194f, 12.074f, 7.856f, 12f, 7.5f, 12f)
            curveToRelative(-0.563f, 0f, -1.082f, 0.186f, -1.5f, 0.5f)
            close()
            moveTo(17.25f, 19f)
            curveTo(16.56f, 19f, 16f, 19.56f, 16f, 20.25f)
            reflectiveCurveToRelative(0.56f, 1.25f, 1.25f, 1.25f)
            horizontalLineToRelative(13.5f)
            curveToRelative(0.69f, 0f, 1.25f, -0.56f, 1.25f, -1.25f)
            reflectiveCurveTo(31.44f, 19f, 30.75f, 19f)
            horizontalLineToRelative(-13.5f)
            close()
            moveTo(46f, 35f)
            curveToRelative(0f, 6.075f, -4.925f, 11f, -11f, 11f)
            reflectiveCurveToRelative(-11f, -4.925f, -11f, -11f)
            reflectiveCurveToRelative(4.925f, -11f, 11f, -11f)
            reflectiveCurveToRelative(11f, 4.925f, 11f, 11f)
            close()
            moveToRelative(-3f, 0f)
            curveToRelative(0f, -0.552f, -0.448f, -1f, -1f, -1f)
            horizontalLineTo(28f)
            curveToRelative(-0.552f, 0f, -1f, 0.448f, -1f, 1f)
            reflectiveCurveToRelative(0.448f, 1f, 1f, 1f)
            horizontalLineToRelative(14f)
            curveToRelative(0.552f, 0f, 1f, -0.448f, 1f, -1f)
            close()
        }
    }.build()
}
