package com.sheshu.fluenticons.regular

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.sheshu.fluenticons.FluentIcons

public val FluentIcons.Regular.DeleteArrowBack16: ImageVector by lazy(LazyThreadSafetyMode.NONE) {
    ImageVector.Builder(
        name = "Regular.DeleteArrowBack16",
        defaultWidth = 16.dp,
        defaultHeight = 16.dp,
        viewportWidth = 16f,
        viewportHeight = 16f
    ).apply {
        path(fill = SolidColor(Color.Black)) {
            moveTo(9f, 3f)
            curveToRelative(0f, -0.552f, -0.448f, -1f, -1f, -1f)
            reflectiveCurveTo(7f, 2.448f, 7f, 3f)
            horizontalLineToRelative(2f)
            close()
            moveTo(6f, 3f)
            curveToRelative(0f, -1.105f, 0.895f, -2f, 2f, -2f)
            reflectiveCurveToRelative(2f, 0.895f, 2f, 2f)
            horizontalLineToRelative(4f)
            curveToRelative(0.276f, 0f, 0.5f, 0.224f, 0.5f, 0.5f)
            reflectiveCurveTo(14.276f, 4f, 14f, 4f)
            horizontalLineToRelative(-0.564f)
            lineToRelative(-0.233f, 1.71f)
            curveToRelative(-0.3f, -0.17f, -0.619f, -0.313f, -0.951f, -0.425f)
            lineTo(12.427f, 4f)
            horizontalLineTo(3.573f)
            lineToRelative(1.187f, 8.703f)
            curveTo(4.86f, 13.446f, 5.495f, 14f, 6.245f, 14f)
            horizontalLineToRelative(0.011f)
            curveToRelative(0.314f, 0.38f, 0.677f, 0.716f, 1.08f, 1f)
            horizontalLineToRelative(-1.09f)
            curveToRelative(-1.25f, 0f, -2.308f, -0.924f, -2.477f, -2.162f)
            lineTo(2.563f, 4f)
            horizontalLineTo(2f)
            curveTo(1.724f, 4f, 1.5f, 3.776f, 1.5f, 3.5f)
            reflectiveCurveTo(1.724f, 3f, 2f, 3f)
            horizontalLineToRelative(4f)
            close()
            moveToRelative(4.5f, 12f)
            curveToRelative(2.485f, 0f, 4.5f, -2.015f, 4.5f, -4.5f)
            reflectiveCurveTo(12.985f, 6f, 10.5f, 6f)
            reflectiveCurveTo(6f, 8.015f, 6f, 10.5f)
            reflectiveCurveTo(8.015f, 15f, 10.5f, 15f)
            close()
            moveTo(9.604f, 8.604f)
            lineTo(8.707f, 9.5f)
            horizontalLineToRelative(1.543f)
            curveToRelative(1.519f, 0f, 2.75f, 1.231f, 2.75f, 2.75f)
            verticalLineToRelative(0.25f)
            curveToRelative(0f, 0.276f, -0.224f, 0.5f, -0.5f, 0.5f)
            reflectiveCurveTo(12f, 12.776f, 12f, 12.5f)
            verticalLineToRelative(-0.25f)
            curveToRelative(0f, -0.966f, -0.784f, -1.75f, -1.75f, -1.75f)
            horizontalLineTo(8.707f)
            lineToRelative(0.896f, 0.896f)
            curveToRelative(0.196f, 0.196f, 0.196f, 0.512f, 0f, 0.708f)
            curveToRelative(-0.195f, 0.195f, -0.511f, 0.195f, -0.707f, 0f)
            lineTo(7.144f, 10.35f)
            curveToRelative(-0.047f, -0.047f, -0.082f, -0.102f, -0.106f, -0.16f)
            curveTo(7.013f, 10.132f, 7f, 10.068f, 7f, 10f)
            reflectiveCurveToRelative(0.013f, -0.132f, 0.038f, -0.191f)
            curveToRelative(0.024f, -0.06f, 0.06f, -0.115f, 0.108f, -0.162f)
            lineToRelative(1.75f, -1.75f)
            curveToRelative(0.196f, -0.196f, 0.512f, -0.196f, 0.707f, 0f)
            curveToRelative(0.196f, 0.195f, 0.196f, 0.511f, 0f, 0.707f)
            close()
        }
    }.build()
}
